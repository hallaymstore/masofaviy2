...
export async function inspectScormZip(buffer){
  ...
  return {zip,launch,fileCount:files.length,standard:manifest?.metadata?.schemaversion||'SCORM 1.2'};
}
...
export function installScorm(app,{mongoose,auth,audit,courseAccess}){
  ...
  const sessionSchema=new mongoose.Schema({tokenHash:{type:String,unique:true,required:true},packageId:{type:id,ref:'ScormPackage',required:true},studentId:{type:id,ref:'User',required:true},expiresAt:{type:Date,required:true}},{timestamps:true});
  ...
  app.post('/api/lms/scorm/:id/launch',auth,wrap(async(req,res)=>{
    ...
    await Session.create({tokenHash:crypto.createHash('sha256').update(token).digest('hex'),packageId:pkg._id,studentId:req.user._id,expiresAt});
  ...
  app.post('/api/lms/scorm/:id/progress',auth,wrap(async(req,res)=>{
    ...
    const allowed=/^cmi\.(core\.(lesson_status|score\.(raw|min|max)|lesson_location|session_time|total_time|exit)|suspend_data)$/;
    const entries=Object.entries(req.body.values||{});if(entries.length>20||entries.some(([k,v])=>!allowed.test(k)||typeof v!=='string'||v.length>4096))throw new Error('SCORM ma’lumoti noto‘g‘ri');
...
export function runtimeShim(token){
  // The iframe has an opaque origin. Only its parent page can send authorized progress writes.
  return `(()=>{const token=${JSON.stringify(token)},values={};let error='0';const defaults={'cmi.core.lesson_status':'not attempted','cmi.core.score.raw':'','cmi.core.lesson_location':'','cmi.suspend_data':''};window.API={LMSInitialize:()=>{parent.postMessage({kind:'scorm-ready',token},'*');return 'true'},LMSFinish:()=>{parent.postMessage({kind:'scorm-progress',token,values},'*');return 'true'},LMSGetValue:k=>{error='0';return values[k]??defaults[k]??''},LMSSetValue:(k,v)=>{if(typeof k!=='string'||!/^cmi\\.(core\\.(lesson_status|score\\.(raw|min|max)|lesson_location|session_time|total_time|exit)|suspend_data)$/.test(k)){error='401';return 'false'}values[k]=String(v).slice(0,4096);error='0';return 'true'},LMSCommit:()=>{parent.postMessage({kind:'scorm-progress',token,values},'*');return 'true'},LMSGetLastError:()=>error,LMSGetErrorString:()=>error==='0'?'No error':'Invalid data model element',LMSGetDiagnostic:()=>error};window.addEventListener('message',e=>{if(e.source!==parent||e.data?.kind!=='scorm-state'||e.data?.token!==token)return;Object.assign(values,e.data.values||{})});})();`;
}
