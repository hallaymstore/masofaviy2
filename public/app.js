...
async function renderStudentAcademic(student,adminMode=false){
  ...
  const transcriptHtml=transcript.results.map(r=>'<div class="lesson-row"><div><b>'+esc(r.courseId?.title||'Fan')+'</b><small>'+esc(r.academicYear)+' · '+esc(r.semester)+'-semestr · '+esc(r.totalScore)+' ball · '+esc(r.letterGrade)+' · '+esc(r.creditsAwarded)+' kredit</small></div></div>').join('')||'<p>Yakuniy akademik natija hali yo‘q.</p>';
