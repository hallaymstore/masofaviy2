...
$('#curriculumTemplate')?.addEventListener('click',()=>{
  const csv='semestr,fan_kodi,fan_nomi,kredit,til,fan_dasturi,amaliyot\n1,MAT101,Oliy matematika,6,uz,https://university.uz/syllabus/MAT101,yoq\n1,INF101,Informatika,5,uz,https://university.uz/syllabus/INF101,ha\n';
...
async function openCourse(id){
  try{const [x,scorm]=await Promise.all([api('/lms/courses/'+id),api('/lms/courses/'+id+'/scorm')]),editor=user.role==='teacher'||['admin','superadmin'].includes(user.role);let html='<article><h2>'+esc(x.course.title)+'</h2><p>'+esc(x.course.code)+' · '+esc(x.course.language)+'</p>';
    ...
    if(user.role==='student')html+='<article><h3>Fikr-mulohaza</h3><p>Baholar faqat kamida beshta talaba javob bergach yig‘ma shaklda ko‘rinadi; shaxsiy javob o‘qituvchiga berilmaydi.</p><button id="courseFeedback">Fan va o‘qituvchini baholash</button> <button id="platformFeedback">Platformani baholash</button></article>';
    if(editor)html+='<article><h3>Fikr-mulohazalar</h3><button id="courseFeedbackStats">Anonim fan statistikasi</button>'+(['admin','superadmin'].includes(user.role)?' <button id="platformFeedbackStats">Platforma statistikasi</button>':'')+'</article>';
    $('#courseDetail').innerHTML=html;
    $('#courseFeedback')?.addEventListener('click',()=>modal('Fan va o‘qituvchi haqida fikr','<p>1 — juda qoniqarsiz, 5 — juda yaxshi. Baholashni keyin yangilashingiz mumkin.</p><label>Fan sifati<input type="number" name="courseRating" min="1" max="5" step="1" required></label><label>O‘qituvchi bilan ishlash<input type="number" name="teacherRating" min="1" max="5" step="1" required></label>',async d=>{await api('/lms/courses/'+id+'/feedback',{method:'POST',body:JSON.stringify(d)});toast('Fikringiz qabul qilindi')}));
    $('#platformFeedback')?.addEventListener('click',()=>modal('Platforma haqida fikr','<label>Platformadan foydalanish qulayligi (1–5)<input type="number" name="platformRating" min="1" max="5" step="1" required></label>',async d=>{await api('/lms/feedback/platform',{method:'POST',body:JSON.stringify(d)});toast('Platforma bahosi qabul qilindi')}));
    $('#courseFeedbackStats')?.addEventListener('click',async()=>{try{const x=await api('/lms/courses/'+id+'/feedback-summary');$('#courseDetail').innerHTML+='<article><h3>Fan bo‘yicha yig‘ma baholar</h3><p>'+esc(x.academicPeriod)+' · '+esc(x.responses)+' talaba javobi</p><p>'+(x.visible?'Fan: '+esc(x.means.courseRating)+'/5 · O‘qituvchi: '+esc(x.means.teacherRating)+'/5':esc(x.reason))+'</p></article>'}catch(e){toast(e.message)}});
    $('#platformFeedbackStats')?.addEventListener('click',async()=>{try{const x=await api('/lms/feedback/platform-summary');$('#courseDetail').innerHTML+='<article><h3>Platforma baholari</h3><p>'+esc(x.academicPeriod)+' · '+esc(x.responses)+' javob</p><p>'+(x.visible?'O‘rtacha baho: '+esc(x.means.platformRating)+'/5':esc(x.reason))+'</p></article>'}catch(e){toast(e.message)}});
...
async function startExamSignals(attemptId){
  ...
  const send=type=>{queue=queue.then(()=>api('/lms/attempts/'+attemptId+'/proctor-events',{method:'POST',body:JSON.stringify({type})})).catch(()=>{});return queue;};
  ...
  stopSignals.flush=()=>queue;
  await queue; // initial camera/microphone readiness must reach the server before the exam form opens
  return stopSignals;
}
