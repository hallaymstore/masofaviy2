...
async function startExamSignals(attemptId){
  let camera=null,microphone=null,timer=null,detector=null,audioContext=null,analyser=null,busy=false,stopped=false,lastSound=0,enteredFullscreen=false;
  const send=type=>api('/lms/attempts/'+attemptId+'/proctor-events',{method:'POST',body:JSON.stringify({type})}).catch(()=>{});
  const visibility=()=>{if(document.hidden)send('page_hidden')},blur=()=>send('window_blur'),offline=()=>send('network_offline'),online=()=>send('network_online');
  const fullscreen=()=>{if(document.fullscreenElement){enteredFullscreen=true;send('fullscreen_enter')}else if(enteredFullscreen)send('fullscreen_exit')};
  document.addEventListener('visibilitychange',visibility);document.addEventListener('fullscreenchange',fullscreen);window.addEventListener('blur',blur);window.addEventListener('offline',offline);window.addEventListener('online',online);
  document.documentElement.requestFullscreen?.().catch(()=>{});
  try{camera=await navigator.mediaDevices.getUserMedia({video:{width:320,height:240},audio:false});send('camera_ready');camera.getVideoTracks().forEach(t=>t.addEventListener('ended',()=>send('camera_track_ended'),{once:true}))}catch{send('camera_unavailable')}
  try{microphone=await navigator.mediaDevices.getUserMedia({video:false,audio:true});send('microphone_ready');microphone.getAudioTracks().forEach(t=>t.addEventListener('ended',()=>send('microphone_track_ended'),{once:true}));const AudioContext=window.AudioContext||window.webkitAudioContext;if(AudioContext){audioContext=new AudioContext();analyser=audioContext.createAnalyser();analyser.fftSize=2048;audioContext.createMediaStreamSource(microphone).connect(analyser)}}catch{send('microphone_unavailable')}
  if(camera){
    ...
    const checkPosition=d=>{const box=d?.boundingBox;if(!box)return;const x=box.xCenter??((box.x??0)+(box.width??0)/2),y=box.yCenter??((box.y??0)+(box.height??0)/2),normalized=x<=1&&y<=1;if(normalized&&(x<0.28||x>0.72||y<0.24||y>0.78))send('face_off_center')};
    try{
      if('FaceDetector'in window)detector={kind:'native',instance:new FaceDetector({fastMode:true,maxDetectedFaces:2})};
        ...
        const instance=new window.FaceDetection({locateFile:file=>'/vendor/face-detection/'+file});instance.setOptions({model:'short',minDetectionConfidence:0.5});instance.onResults(result=>{if(stopped)return;const list=result.detections||[],n=list.length;if(n===0)send('face_missing');if(n>1)send('multiple_faces');if(n===1)checkPosition(list[0])});detector={kind:'mediapipe',instance}
      }
    ...
    timer=setInterval(async()=>{
      ...
      try{
        if(detector&&video.readyState>=2){
          if(detector.kind==='native'){const list=await detector.instance.detect(video),n=list.length;if(n===0)send('face_missing');if(n>1)send('multiple_faces');if(n===1){const b=list[0].boundingBox;if(b){const x=(b.x+b.width/2)/video.videoWidth,y=(b.y+b.height/2)/video.videoHeight;if(x<0.28||x>0.72||y<0.24||y>0.78)send('face_off_center')}}}
  ...
  return ()=>{if(stopped)return;stopped=true;clearInterval(timer);detector?.instance.close?.();camera?.getTracks().forEach(t=>t.stop());microphone?.getTracks().forEach(t=>t.stop());audioContext?.close().catch(()=>{});if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});document.removeEventListener('visibilitychange',visibility);document.removeEventListener('fullscreenchange',fullscreen);window.removeEventListener('blur',blur);window.removeEventListener('offline',offline);window.removeEventListener('online',online)};
}
