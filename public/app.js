...
async function openCourse(id){
  try{const x=await api('/lms/courses/'+id),editor=user.role==='teacher'||['admin','superadmin'].includes(user.role);let html='<article><h2>'+esc(x.course.title)+'</h2><p>'+esc(x.course.code)+' · '+esc(x.course.language)+'</p>';
    ...
    if(editor)html+='<button id="newResource">+ Material</button> <button id="newAssignment">+ Topshiriq</button>';
    ...
    html+='<h3>Testlar</h3>'+(x.quizzes.map(q=>'<p>'+esc(q.title)+' <button data-quiz="'+esc(q._id)+'">Boshlash</button></p>').join('')||'<p>Test yo‘q</p>')+'</article>';
