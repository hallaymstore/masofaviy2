...
window.addEventListener('popstate',async()=>{if(!token)return;const deep=location.hash.startsWith('#video=')?decodeURIComponent(location.hash.slice(7)):'';if(deep){try{await openVideoLesson(deep,false)}catch{go('videos')}}else if(activeVideoId){clearWatchPage();go('videos');loadVideoLessons()}});
hydrateIcons();if('serviceWorker' in navigator)navigator.serviceWorker.register('/sw.js');start();

async function loadCourses(){
  try{const rows=await api('/lms/courses');$('#courseDetail').innerHTML='';$('#courseList').innerHTML=rows.map(c=>'<article><h2>'+esc(c.title)+'</h2><p>'+esc(c.code)+' · '+esc(c.language)+' · '+esc(c.groupId?.name||'')+' · '+esc(c.teacherId?.fullName||'')+'</p><button class="primary" data-course="'+esc(c._id)+'">Ochish</button></article>').join('')||'<div class="empty">Hozircha fanlar biriktirilmagan.</div>';all('[data-course]').forEach(b=>b.onclick=()=>openCourse(b.dataset.course))}catch(e){toast(e.message)}
}
async function openCourse(id){
  try{const [x,scorm]=await Promise.all([api('/lms/courses/'+id),api('/lms/courses/'+id+'/scorm')]),editor=user.role==='teacher'||['admin','superadmin'].includes(user.role);let html='<article><h2>'+esc(x.course.title)+'</h2><p>'+esc(x.course.code)+' · '+esc(x.course.language)+'</p>';
    ...
    html+='<h3>Materiallar</h3>'+(x.resources.map(r=>'<p><a href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer">'+esc(r.title)+' ↗</a> · '+esc(r.kind)+'</p>').join('')||'<p>Material yo‘q</p>');
    if(editor)html+='<button id="newResource">+ Material</button> <button id="newAssignment">+ Topshiriq</button> <button id="newQuiz">+ Test</button>';
    html+='<h3>Topshiriqlar</h3>'+(x.assignments.map(a=>'<div class="lesson-row"><div><b>'+esc(a.title)+'</b><p>'+esc(a.instructions)+'</p><small>Muddat: '+esc(a.dueAt?new Date(a.dueAt).toLocaleString('uz-UZ'):'belgilanmagan')+'</small></div><button data-assignment="'+esc(a._id)+'">'+(user.role==='student'?'Javob berish':'Javoblarni ko‘rish')+'</button></div>').join('')||'<p>Topshiriq yo‘q</p>');
...
async function startExamSignals(attemptId){let stream=null,timer=null,detector=null,stopped=false;const send=type=>api('/lms/attempts/'+attemptId+'/proctor-events',{method:'POST',body:JSON.stringify({type})}).catch(()=>{});const visibility=()=>{if(document.hidden)send('page_hidden')},blur=()=>send('window_blur');document.addEventListener('visibilitychange',visibility);window.addEventListener('blur',blur);try{stream=await navigator.mediaDevices.getUserMedia({video:{width:320,height:240},audio:true});send('camera_ready');if('FaceDetector'in window)detector=new FaceDetector({fastMode:true,maxDetectedFaces:2});else send('face_detector_unavailable')}catch{send('camera_unavailable');send('microphone_unavailable')}
  if(stream&&detector){const video=document.createElement('video');video.srcObject=stream;video.muted=true;video.playsInline=true;await video.play().catch(()=>{});timer=setInterval(async()=>{if(stopped)return;try{const faces=await detector.detect(video);if(faces.length===0)send('face_missing');if(faces.length>1)send('multiple_faces')}catch{}},8000)}
  return ()=>{if(stopped)return;stopped=true;clearInterval(timer);stream?.getTracks().forEach(t=>t.stop());document.removeEventListener('visibilitychange',visibility);window.removeEventListener('blur',blur)};
}
